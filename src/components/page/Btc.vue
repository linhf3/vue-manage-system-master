...
<!--                <el-table-column label="数值">
                ...
                <el-table-column label="WR8">
                  <template slot-scope="scope">
                    <div v-if="scope.row.positiveNegativeFlag == -1" style="color:red;font-weight:bold;">{{scope.row.d8}}</div>
                    <div v-if="scope.row.positiveNegativeFlag == 0" >{{scope.row.d8}}</div>
                    <div v-if="scope.row.positiveNegativeFlag == 1" style="color:green;font-weight:bold;">{{scope.row.d8}}</div>
                  </template>
                </el-table-column>
                <el-table-column label="WR14">
                  <template slot-scope="scope">
                    <div v-if="scope.row.positiveNegativeFlag == -1" style="color:red;font-weight:bold;">{{scope.row.d14}}</div>
                    <div v-if="scope.row.positiveNegativeFlag == 0" >{{scope.row.d14}}</div>
                    <div v-if="scope.row.positiveNegativeFlag == 1" style="color:green;font-weight:bold;">{{scope.row.d14}}</div>
                  </template>
                </el-table-column>-->
...
export default {
    ...
    methods: {
      ...
      getStockData() { // 从服务端加载数据的函数
        this.$http.get('/godwealth/api/btc/getBtc/').then((response)=> {
          if (response.status != 200){
            alert(response.data.response.data.message);
            this.$options.methods.stopInterval()
          }
          debugger
          this.stockList =  response.data.resultList;
          debugger
          //alert(list);
        ...
        getFuturesData() { // 从服务端加载数据的函数
            this.$http.get('/godwealth/api/btc/getBtc/').then((response)=> {
              ...
              debugger
              this.futuresList =  response.data.resultList;
